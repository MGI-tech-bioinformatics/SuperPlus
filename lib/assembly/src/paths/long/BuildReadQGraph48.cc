...
public:
    ...
    { if ( pp1.getEdgeID() == pp2.getEdgeID() ) return true;
      ...
      SubKmer k2 = pp2.isRC() ? SubKmer(e2.rcbegin()) : SubKmer(e2.begin());
      return k1==k2; }
...
//        if ( part.getEdgeOffset() == 0 && part.getLength() < 5) continue;
        size_t idx = part.getEdgeID().val(); // old edge ID
        path.push_back(part.isRC()?mRevEdgeXlat[idx]:mFwdEdgeXlat[idx]); // store new edge ID
        pLast = &part; }
      if ( path.empty() )
