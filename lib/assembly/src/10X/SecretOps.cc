...
{    cout << Date( ) << ": start AllTinks" << endl;
	 ...
	 Sort(rbc);
	 int halfN50rbc = N50(rbc)/2;

	 cout << Date() << ": min_barcode threshold set as " << min(min_reads, halfN50rbc) << endl;

     double clock = WallClockTime( );
     vec<int> gbc;
     for ( int b = 1; b < NBC; b++ )
     {    int64_t nbc = bci[b+1] - bci[b];
          if ( (nbc < min_reads && nbc < halfN50rbc) || nbc > max_reads ) continue;
          gbc.push_back(b);    }
     ...
     for ( int d = 0; d < npasses; d++ )
     {
          ...
          {    // if ( starts[i] > 0 && ep[ starts[i] ] == ep[ starts[i] - 1 ] )
               while ( starts[i] > 0 && ep[ starts[i] ] == ep[ starts[i] - 1 ] )
                    starts[i]--;    }
