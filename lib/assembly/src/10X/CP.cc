...
{    RunTime( );
     ...
     {    const Bool DJANGO = False;
          ...
          // less than 20 kb.
          {
			   ...
			   OUTDIR = DIR + "/" + "records" + WRITE_SUB;
			   Mkdir777(OUTDIR);
			   BinaryWriter::writeFile(OUTDIR + "/lwml", (int)lw_mean_mol_length);
			   OUTDIR = DIR + "/" + "final" + WRITE_SUB;
          }
     ...
     {    
          ...
          String R, S;
          Stackaroo( bases, quals, hb, inv, xpaths, xpaths_index, D, dinv, S, R,
     ...
     {    
          ...
          // initial estimated molecule length.
          double lw_mean_mol_length = 100000.0;
          for (int pass = 0; pass < 1; ++pass) {
              
              Star( hb, inv, dup, bc, paths, D, dinv, dpaths, ebcx,
                    qept, MIN_ADVANTAGE, OO, DJANGO, False );
              ...
              CleanupCore( D, dinv );
			  //cout << Date() << ": mol_lwml = "
				  //<< ToStringAddCommas(mol_lwml) << endl;
          }
          /*
          {
              vec<int> to_left, to_right;
              ...
              int bc_only_gap_cnt = 0, broken_line_cnt = 0, line_broken_cnt = 0;
              int long_line_cnt = 0;
              for (int i = 0; i < dlines.isize(); ++i) {
                  if (llens[i] >= 4000) {
                      ++long_line_cnt;
                  ...
                  if (bc_only_gap_local) {
                      broken_line_cnt += bc_only_gap_local + 1;
                      ++line_broken_cnt;
                  }
              }
              cout << Date( ) << ": bc_only gap left in lines: " << bc_only_gap_cnt 
                   << " there are " << dlines.size() << " lines in total, including " << long_line_cnt << " long ones" << endl
                   << line_broken_cnt << " line got broken, forming " << broken_line_cnt
                   << " broken lines" << endl;
              bc_only_gap_cnt = 0;
              for (int i = 0; i < D.E(); ++i) {
                  if (IsBarcodeOnlyGap( D.O(i))) {
                      ++bc_only_gap_cnt;
                  }
              }
              cout << Date( ) << " bc_only gap left in supergraph: " << bc_only_gap_cnt << endl;
          }
          */
#if 0
          /*
          ...
          ScaffoldMe( hb, inv, dup, bc, paths, D, dinv, dpaths, ebcx,
                  qept, MIN_ADVANTAGE, OO, DJANGO, BC_REQUIRE, 
                  BC_FLANK, BC_IGNORE, False, True );
