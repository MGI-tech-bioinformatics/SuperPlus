...
void MemoryOrderAndOrientN( 
     int pivot, // id needs to find a best right
     // lines to OO
     const vec<int>& L0,                    // list of line ids
     ...
     // answer:
     double& advantage, vec<int>& L,
     Bool is_last ) // position to insert, true --> push_back, false --> push_front
{
     // Create candidates.
     ...
     {    for ( int l = 0; l < Lt[j].isize( ); l++ )
          ...
          // add pivot id
          if (is_last) {
              Lt[j].push_back(pivot);
          } else {
              Lt[j].push_front(pivot);
          }
          ad[j] = MemoryScoreOrder( Lt[j], lbp, llens, M, memory );    }
     vec<int> ids( N, vec<int>::IDENTITY );
     SortSync( ad, ids );
...
void MemoryOrderAndOrientN( 
     const vec<int> &pivot, // id needs to find a best right
     // lines to OO
     const vec<int>& L0,                    // list of line ids
     ...
     double& advantage, vec<int>& L )
{
     ...
     {    for ( int l = 0; l < Lt[j].isize( ); l++ )
               if ( !fwt[j][l] ) Lt[j][l] = linv[ Lt[j][l] ];
          // add pivot id
          Lt[j].push_front(pivot[0]);
          Lt[j].push_back(pivot[1]);
          ad[j] = MemoryScoreOrder( Lt[j], lbp, llens, M, memory );    }
     vec<int> ids( N, vec<int>::IDENTITY );
     SortSync( ad, ids );
     L = Lt[ ids[0] ];
